#!/usr/bin/env python3
"""
Polygon.io XGBoost HFT Trading System - Clean Version
====================================================
...
class PricePredictionModel:
    ...
    def prepare_features(self, df: pd.DataFrame, ticker: str) -> pd.DataFrame:
        ...
        df['log_returns'] = np.log(df['vwap'] / df['vwap'].shift(1)).fillna(0)
        ...
        # Simple lag features
        for lag in [1, 2]:
            df[f'vwap_lag_{lag}'] = df['vwap'].shift(lag).fillna(method='bfill')
    ...
    def train_model(self, df: pd.DataFrame) -> bool:
        ...
        missing_features = [col for col in self.feature_cols if col not in df_work.columns]
        ...
        # Proper parameter grid for comprehensive search
        param_grid = {
        ...
        try:
            ...
            xgb = XGBRegressor(
                ...
                tree_method='hist',
                device='cpu',
                n_jobs=-5               # Use most cores, leave some for user
            )
            ...
            print(f"Model trained: MSE={mse:.6f}, Features={len(self.feature_cols)}, Data={len(X)}")
            return True
    ...
    def predict_next_vwap(self, df: pd.DataFrame) -> float:
        ...
        try:
            ...
            features = features.reshape(1, -1)  # Ensure 2D array for single prediction
            predicted_vwap = self.model.predict(features)[0]
...
class PositionManager:
    ...
    def check_safety_net_sell(self, ticker: str, current_vwap: float) -> bool:
        ...
        if drop_from_max >= threshold:
            print(f"STOP LOSS: Safety net: {ticker} dropped {drop_from_max:.1f}BP from max")
            return True
    ...
    def calculate_position_size(self, ticker: str, current_vwap: float, is_pyramid: bool = False) -> int:
        ...
        if is_pyramid:
            max_spend = self.allocations[ticker] * 0.5  # 50% for pyramid
        else:
    ...
    def execute_buy(self, ticker: str, current_vwap: float, is_pyramid: bool = False) -> bool:
        ...
        action = f"PYRAMID Pyramid {self.pyramid_history[ticker]}" if is_pyramid else "BUY Buy"
        print(f"{action} {ticker}: {shares} shares @ ${current_vwap:.2f}")
        
        return True
    
    def execute_sell(self, ticker: str, current_vwap: float) -> bool:
        ...
        print(f"{emoji} Sold {ticker}: {shares} shares @ ${current_vwap:.2f} | P&L: ${profit_loss:.2f} ({pnl_pct:+.2f}%)")
    ...
    def make_decision(self, prediction_signal: int, technical_signals: Dict[str, int], 
        ...
        if not holding_position:
            ...
            if sell_count >= 3:
                ...
                # Pyramid: +2BP prediction + 2/3 other indicators
...
class PolygonXGBoostTrader:
    ...
    def execute_trading_cycle(self, current_data: Dict[str, pd.DataFrame]) -> Dict:
        ...
        for ticker in self.tickers:
            ...
            if self.position_manager.check_safety_net_sell(ticker, current_vwap):
                self.position_manager.execute_sell(ticker, current_vwap)
    ...
    def run_weekly_update(self) -> Dict:
        """Weekly retraining and trading"""
        print("\n" + "="*60)
        print("WEEKLY WEEKLY UPDATE")
        print("="*60)
        ...
        for ticker in self.tickers:
            if self.models[ticker].is_trained:
                model_info = {
                    'feature_count': len(self.models[ticker].feature_cols),
        ...
        for ticker in self.tickers:
            if self.models[ticker].is_trained:
                self.models[ticker].model.save_model(f"{week_folder}/{ticker}_model.json")
    ...
    def _run_minute_by_minute_backtest(self, test_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        for current_time in tqdm(timestamps, desc="Simulating minutes", unit="min"):
            ...
            for ticker in self.tickers:
                ...
                # 1. Safety net check (but preserve prediction data)
                if self.position_manager.check_safety_net_sell(ticker, current_vwap):
                    if self.position_manager.execute_sell(ticker, current_vwap):
                ...
                # 2. Ensemble decision (using already calculated prediction/signals)
                holding_position = self.position_manager.positions[ticker] > 0
                decision = self.ensemble_decision.make_decision(
                    prediction_signal, technical_signals, holding_position, predicted_bp
                )
            ...
            capital_utilization = (self.position_manager.initial_capital - current_cash) / self.position_manager.initial_capital * 100
            ...
            for ticker in self.tickers:
                ...
                if action != 'Hold':
                    ...
                    minute_actions.append(f"{ticker}: {action} @${current_vwaps.get(ticker, 0):.2f} (Pred={pred_bp:.1f}BP, Signals={signals})")
        ...
        if len(results_df) > 0:
            ...
            if not trades.empty:
                ...
                # Reshape data to ticker, pred_bp, action format
                clean_trades = []
                for _, row in trades.head(10).iterrows():
                    for ticker in self.tickers:
                        ...
                        if pd.notna(action):  # If there's an action recorded
                            clean_trades.append({
                                'ticker': ticker,
                                'pred_bp': pred_bp,
                                'action': action
...
def main():
    ...
    TICKERS = ["TSLA", "NVDA", "AAPL", "META", "MCD", "PEP", "PFE", "PLTR", "UBER", "BAC", "V", "NFLX"]
    ...
    # Full allocation list for production
    ALLOCATIONS = {
        "TSLA": 25000,      # Tech
        "NVDA": 25000,      # Tech  
        "AAPL": 25000,      # Tech
    ...
    # Run proper backtest
    print("\nRUNNING BACKTEST")
    end_date = datetime(2025, 8, 22)  # Use date where all tickers have data
    start_date = end_date - timedelta(weeks=2)  # 2 weeks test period
    
    # Run minute-by-minute backtest
    results_df = trader.run_backtest(
    ...
    # Save results in main run folder
    if not results_df.empty:
        # Save main backtest results
        results_df.to_csv(f'{trader.run_folder}/backtest_results.csv', index=False)
        
        # Save portfolio tracking data
        portfolio_cols = ['timestamp_str', 'current_cash', 'portfolio_value', 'total_fees_paid', 
                         'profit', 'pure_profit', 'total_shares_owned', 'active_positions', 
                         'capital_utilization', 'drawdown_pct', 'total_trades', 'return_pct']
        portfolio_tracking = results_df[portfolio_cols].copy()
        portfolio_tracking.to_csv(f'{trader.run_folder}/portfolio_performance.csv', index=False)
        ...
        backtest_config = {
            ...
            'allocations': trader.position_manager.allocations,
            'start_date': start_date.isoformat(),
            'end_date': end_date.isoformat(),
            'train_weeks': train_weeks,
            'test_weeks': test_weeks,
            'final_portfolio_value': portfolio_tracking['portfolio_value'].iloc[-1] if len(portfolio_tracking) > 0 else 0,
            'total_return_pct': portfolio_tracking['return_pct'].iloc[-1] if len(portfolio_tracking) > 0 else 0,
            'total_trades': portfolio_tracking['total_trades'].iloc[-1] if len(portfolio_tracking) > 0 else 0,
            'total_fees': portfolio_tracking['total_fees_paid'].iloc[-1] if len(portfolio_tracking) > 0 else 0
        }
        
        with open(f'{trader.run_folder}/backtest_config.json', 'w') as f:
            json.dump(backtest_config, f, indent=2)
        
        print(f"\nAll results saved to {trader.run_folder}/")
        print(f"  - backtest_results.csv (full trading data)")
        print(f"  - portfolio_performance.csv (minute-by-minute metrics)")  
        print(f"  - backtest_config.json (configuration and summary)")
        print(f"  - week_YYYYMMDD/ folders (individual week data and models)")
    
    # Optional: Run weekly updates as well
